...
import { UsersFilters } from './customers-filters';
import { UsersTable } from './customers-table';
import type { User } from './customers-table';
...
export function UsersView(): React.JSX.Element {
  ...
  const filtered = React.useMemo(() => {
    ...
    return USERS.filter(
      (u) =>
        ...
        `${u.address.city}, ${u.address.state}, ${u.address.country}`.toLowerCase().includes(q)
    );
  }, [query]);
  ...
  return (
    <Stack spacing={3}>
      ...
      <UsersFilters query={query} onQueryChange={setQuery} />
      <UsersTable
